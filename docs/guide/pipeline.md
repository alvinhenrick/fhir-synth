...
Use them directly with `--compiled-program`:

```bash
fhir-synth generate "5 diabetic patients" \
  --pipeline dspy --compiled-program examples/bootstrap_few_shot.json

# Or with MIPROv2
fhir-synth generate "5 diabetic patients" \
  --pipeline dspy --compiled-program examples/miprov2.json
```
